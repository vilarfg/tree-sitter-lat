  ...
  rules: {
    ...
    definition: ($) =>
      seq(
        ...
        choice(seq(w.colon, $._expr), $.namespace)
      ),

    namespace: ($) => seq(w.oc, optional($._namespace), w.cc),
    _namespace: ($) => seq($.definition, repeat(seq(w.optComma, $.definition)), w.optComma),
    ...
    minus: ($) => prec.left(80, seq('-', $._expr)),

    exp: ($) => prec.right(70, seq(alias($._expr, $.left), '^', alias($._expr, $.right))),
    mul: ($) =>
      prec.left(60, seq(alias($._expr, $.left), choice('×', '*'), alias($._expr, $.right))),
    div: ($) =>
      prec.left(60, seq(alias($._expr, $.left), choice('÷', '/'), alias($._expr, $.right))),
    rem: ($) =>
      prec.left(60, seq(alias($._expr, $.left), choice(w.remainder), alias($._expr, $.right))),
    mod: ($) =>
      prec.left(60, seq(alias($._expr, $.left), choice(w.mod, w.modulo), alias($._expr, $.right))),
    add: ($) => prec.left(50, seq(alias($._expr, $.left), '+', alias($._expr, $.right))),
    sub: ($) => prec.left(50, seq(alias($._expr, $.left), '-', alias($._expr, $.right))),
    lt: ($) => prec.left(40, seq(alias($._expr, $.left), '<', alias($._expr, $.right))),
    lte: ($) =>
      prec.left(40, seq(alias($._expr, $.left), choice('≤', '<='), alias($._expr, $.right))),
    gte: ($) =>
      prec.left(40, seq(alias($._expr, $.left), choice('≥', '>='), alias($._expr, $.right))),
    gt: ($) => prec.left(40, seq(alias($._expr, $.left), '>', alias($._expr, $.right))),
    in: ($) => prec.left(40, seq(alias($._expr, $.left), w.in_op, alias($._expr, $.right))),
    out: ($) => prec.left(40, seq(alias($._expr, $.left), w.out, alias($._expr, $.right))),
    is: ($) => prec.left(40, seq(alias($._expr, $.left), w.is, alias($.type, $.right))),
    neq: ($) =>
      prec.left(
        30,
        seq(alias($._expr, $.left), choice('≠', '<>', '!=', '!=='), alias($._expr, $.right))
      ),
    eq: ($) =>
      prec.left(30, seq(alias($._expr, $.left), choice('=', '==', '==='), alias($._expr, $.right))),
    mtc: ($) => prec.left(25, seq(alias($._expr, $.left), w.matches, alias($._expr, $.right))),
    and: ($) => prec.left(20, seq(alias($._expr, $.left), w.and, alias($._expr, $.right))),
    or: ($) => prec.left(10, seq(alias($._expr, $.left), w.or, alias($._expr, $.right))),
    ...
    cases: ($) => seq(w.oc, optional(seq($.case, repeat(seq(w.comma, $.case)), w.optComma)), w.cc),
    ...
    _number: ($) =>
      choice(
        ...
        seq(alias(/0(U|u)/, 'base'), choice(alias(token.immediate(/[^\\]/u), $.uni), $.number_esc)),
    ...
    drop: ($) =>
      seq('_', optional(alias(token(seq(/[1-9]/, repeat(seq(optional('_'), /[0-9]/)))), $.places))),
